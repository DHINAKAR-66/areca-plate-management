          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
            <div className="card">
              ...
              <p className="text-2xl font-bold text-gray-900 dark:text-white">
                {data.production.reduce((sum, p) => sum + Number(p.total), 0)}
              </p>
            </div>
            <div className="card">
              <p className="text-sm text-gray-500 dark:text-gray-400">Total Sales</p>
              <p className="text-2xl font-bold text-gray-900 dark:text-white">
                ₹{data.sales.reduce((sum, s) => sum + Number(s.total), 0).toFixed(2)}
              </p>
            </div>
            <div className="card">
              <p className="text-sm text-gray-500 dark:text-gray-400">Total Bills</p>
              <p className="text-2xl font-bold text-gray-900 dark:text-white">
                {data.sales.reduce((sum, s) => sum + Number(s.bill_count), 0)}
              </p>
            </div>
            <div className="card">
              <p className="text-sm text-gray-500 dark:text-gray-400">Electricity Used</p>
              <p className="text-2xl font-bold text-gray-900 dark:text-white">
                {data.electricity.reduce((sum, e) => sum + Number(e.daily_consumption), 0)} units
              </p>
            </div>
          </div>
